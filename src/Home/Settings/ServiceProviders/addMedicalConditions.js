...
//01 July,2019.
//To Add  Medical conditions in Accomodation preferences in Service provider preferences.
...
export default class AddMedicalConditions extends Component {
    ...
    doneTapped() {
        ...
        if (conditionType == 'Select Condition' || conditionType == '') {
            ...
            Toast.show("Please enter symptom.")
    ...
    addMedicalCondition() {
        ...
        //endpoint for medical conditins
        const endPoints = 'insert_or_update_medical_conditions';

        //api url
        const addMedicationUrl = apiUrl + 'subscriber/medicalcondition/' + endPoints;
        ...
        httpBody = {
            ...
            "subscriberMedicalConditionList": [
                {
                    "medicalConditionId": "",
                    "medicalConditionIntensity": this.state.intensityRisk,
                    "medicalConditionMedicalContact": "",
                    "medicalConditionMedication": this.state.medicationOptionsValue,
                    "medicalConditionNotes": this.state.instructions,
                    "medicalConditionSpecificity": this.state.specificity,
                    "medicalConditionSymptoms": this.state.symptoms,
                    "medicalConditionType": this.state.conditionType
                }
            ]
        }

        // // console.log('add medication body:data:--' + JSON.stringify(httpBody));
        // // console.log('add medication url:-' + addMedicationUrl)

        fetch(addMedicationUrl, {
            method: 'POST',
            headers: headersData,
            ...
            .then((responseJson) => {
                // // console.log('response for medical condition:' + JSON.stringify(responseJson))
                if (responseJson.status == '00') {
                    this.setState({ isLoading: false });
                    Toast.show("Medical condition added successfully.")
                    this.props.navigation.pop();
                }
                else {
                    this.setState({ isLoading: false });
                    Toast.show("Medical condition not added. Please try again.")
                }
            })
            .catch((error) => {
                this.setState({ isLoading: false });
    ...
    render() {
        let conditionTypeOptions = [{ value: 'Cholestrol' }, { value: 'Blood Pressure' }, { value: 'Cardiac' }, { value: 'Joints & Bones' },],
        ...
        return (
            <SafeAreaView style={{ flex: 1, backgroundColor: '#e2e2f2' }}>
                ...
                <View style={{ height: Platform.OS === 'ios' ? height * 0.070 : height * 0.070, flexDirection: 'row', backgroundColor: '#2e294f' }}>
                    <View style={{ flex: 9, }}>
                        <TouchableOpacity style={{ flex: 1, justifyContent: 'flex-start', flexDirection: 'row', alignItems: 'center' }}
                            ...
                            <Text style={{ color: '#ecc34d', fontSize: 18, fontWeight: 'bold' }}>Add Medical Conditions</Text>
                        </TouchableOpacity>
                    </View>
                ...
                <KeyboardAvoidingView behavior="padding" enabled>    
                    <ScrollView showsVerticalScrollIndicator={false}  style={{margin: 6}}>
                        <View style={{flex: 9, backgroundColor: '#fff', borderRadius: 4}}>
                            ...
                            />
                            <View style={{height: height*0.25}}></View>
                        </View>
                    </ScrollView>
                </KeyboardAvoidingView>
            </SafeAreaView>
