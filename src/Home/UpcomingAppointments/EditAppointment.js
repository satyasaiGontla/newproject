...
export default class EditAppointmentScreen extends Component {
    ...
    setVariables() {
        const appointmentDetails = this.props.navigation.state.params.selectedAppointment;
      // console.log(JSON.stringify(appointmentDetails));
      // // console.log(JSON.stringify(orderDate));
        this.setState({
            date: appointmentDetails.bookingDate,
            time: appointmentDetails.bookingStartTime,
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                <View style={styles.view}>
                    ...
                    <View style={{marginLeft:10,justifyContent:'center',alignItems:'flex-end',}}>
                        <Text style={{color:'#000',fontSize:15,fontWeight:'bold'}}>{Moment(this.state.date).format('MMM DD')}, {this.state.time}</Text>
                    </View>
                    <View style={{flex:1,marginLeft:10,alignItems:'flex-end',justifyContent:'center'}}>
                        <TouchableOpacity
                            onPress={()=> this.props.navigation.navigate('SlotBookingScreen',{appointmentDetails:this.state.appointmentDetails})}>
                ...
                <View style={styles.view}>
                    <View style={{flex:2,marginLeft:10,justifyContent:'center'}}>
                        {/* <TouchableOpacity> */}
                            <Text style={{color:'#990000',fontSize:15,fontWeight:'bold'}}>Cancel Appointment</Text>
                        {/* </TouchableOpacity> */}
                    </View>
                </View>
                <TouchableOpacity
                    style={[styles.buttonStyle,{backgroundColor:colors.appThemeColor,marginTop:40}]}>
                    <Text style={{color:'#fff',fontSize:15}}>Confirm Changes</Text>
                </TouchableOpacity>
                <TouchableOpacity
